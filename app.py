    ...
    if secret == "4a05a3bdd6c94bf5a94a4e6d3cb16822":
        ...
        if request.method == "POST":
            ...
            if form.validate() or valid:
                ...
                session['sf'] = dict(form.sf.choices).get(form.sf.data)
                session['propagation_delay_model'] = dict(form.prop_delay.choices).get(form.prop_delay.data)
                session['radio_environment'] = str(
                    dict(form.radio_environment.choices).get(form.radio_environment.data))
                session['cyclic_redundacy_check'] = str(form.cyclic_redundacy_check.data)
                session['coding_rate'] = str(form.coding_rate.data)
                session['confirmed_traffic'] = str(form.confirmed_traffic.data)
                session['min_BE'] = str(form.min_BE.data)
                    ...
                    if threadID == 1:
                        try:
                            ...
                            if os.environ['TRAFFICPROF'] == "cbr":
                                ...
                                if os.environ['NETWORK'] == "Wi-Fi 802.11ac":
                                    output = _check_output(
                                        cd_ns3_dir + './waf --jobs=2 --run "wifi-periodic --distance=$DISTANCE --nGW=$NUMGATEWAYS --simulationTime=$SIMULATION_TIME --nWifi=$NUMDEVICES --trafficDirection=$TRAFFICDIR --payloadSize=$PACKETSIZE --period=$LOADFREQ --hiddenStations=$HIDDENDEVICES --radioEnvironment=$RADIOENVIRONMENT --spatialStreams=$SPATIALSTREAMS --txCurrent=$TXCURRENT --rxCurrent=$RXCURRENT --idleCurrent=$IDLECURRENT --ccaBusyCurrent=$CCABUSYCURRENT --MCS=$MCS --channelWidth=$BANDWIDTH --propDelay=$PROPDELAY --spatialStreams=$SPATIALSTREAMS --batteryCap=$BATTERYCAP --voltage=$VOLTAGE 2> log.txt"')
                                    with open("log.txt", "w") as text_file:
                                        text_file.write(output)
                                    _check_output(
                                        'cat "log.txt" | grep -e "client sent 1023 bytes" -e "server received 1023 bytes from" > "log-parsed.txt";')
                                    ...
                                    if os.environ['TRAFFICPROF'] == "cbr":
                                        output = _check_output(
                                            cd_ns3_dir + './waf --jobs=2 --run "6lowpan-cbr --distance=$DISTANCE --nGW=$NUMGATEWAYS --simulationTime=$SIMULATION_TIME --nSta=$NUMDEVICES --packetSize=$PACKETSIZE --dataRate=$MEANLOAD --min_BE=$MINBE --max_BE=$MAXBE --csma_backoffs=$CSMABACKOFFS --maxFrameRetries=$MAXFRAMERETRIES --voltage=$VOLTAGE --txCurrent=$TXCURRENT --rxCurrent=$RXCURRENT --idleCurrent=$IDLECURRENT 2> log.txt"')
                                    else:
                                        output = _check_output(
                                    ...
                                    output = _check_output(
                                        'cd static/ns3-halow; ./waf --jobs=2 --run "rca --rho=$DISTANCE --nGW=$NUMGATEWAYS --simulationTime=$SIMULATION_TIME --Nsta=$NUMDEVICES --payloadSize=$PACKETSIZE --trafficInterval=$LOADFREQ --mcs=$MCS --bandWidth=$BANDWIDTH --voltage=$VOLTAGE --txCurrent=$TXCURRENT --rxCurrent=$RXCURRENT --idleCurrent=$IDLECURRENT" 2> log.txt')
                                    with open("log.txt", "w") as text_file:
                                        text_file.write(output)
                                    _check_output(
                ...
                try:
                    ...
                    if (network == 'LoRaWAN'):
                        ...
                        energy = float(lines[i + 2].split()[-1])
                        capacity = (float(session['battery_capacity']) / 1000.0) * float(session['voltage']) * 3600
                        battery_lifetime = round(
                            ((capacity / energy) * float(session['simulation_time'])) / 86400 / 365, 2)
                        energy = round(energy, 2)
                        session['energy_consumption'] = energy
                        latency = float(latency) * 1000
                        session['latency'] = str(latency)  # To get in ms
