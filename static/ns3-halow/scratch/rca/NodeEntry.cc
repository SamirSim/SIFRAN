...
NodeEntry::NodeEntry(int id, Statistics* stats, Ptr<Node> node, Ptr<NetDevice> device, Configuration config) :
				id(id), stats(stats), node(node), device(device), config(config) {
}
...
void NodeEntry::OnPhyStateChange(std::string context, const Time start,	const Time duration, const WifiPhy::State state) {
	...
	stats->get(this->id).EnergyRxIdle = stats->get(this->id).TotalRxTime.GetSeconds() * config.rxCurrent + stats->get(this->id).TotalIdleTime.GetSeconds() * config.idleCurrent;
	stats->get(this->id).EnergyTx = stats->get(this->id).TotalTxTime.GetSeconds() * config.txCurrent;
